...
const Nav: React.FC<NavProps> = ({
  ...
  return (
    <nav className="sticky top-0 left-0 z-10 flex items-center justify-around w-screen h-16 bg-green-800 shadow">
      ...
      <input
        type="text"
        className="px-4 py-2 rounded-lg focus:outline-none focus:border-green-800"
        placeholder="Search"
        value={searchQuery}
      ...
      <div className="flex items-center gap-5">
        ...
        <div className="relative" onClick={handleToggleSidebar}>
          <div className="absolute w-5 h-5 bg-red-500 rounded-full bottom-3 left-4">
            <p className="text-sm text-center text-white">{cartItemCount}</p>
          </div>
          <svg
